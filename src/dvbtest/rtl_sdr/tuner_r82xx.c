...
//[  604.169750] r820t 2-003a: selecting the delivery system
//[  604.182381] r820t 2-003a: r820t_write: i2c wr reg=0c len=1: f1
rc = r82xx_write_reg(priv, 0x0c, 0xf1);
	if (rc < 0)
		return rc; else fprintf(stderr, "ok\n");
//[  604.194854] r820t 2-003a: r820t_write: i2c wr reg=13 len=1: 31
rc = r82xx_write_reg(priv, 0x13, 0x31);
	if (rc < 0)
		return rc;
//[  604.205241] r820t 2-003a: r820t_write: i2c wr reg=1d len=1: 86
rc = r82xx_write_reg(priv, 0x1d, 0x86);
	if (rc < 0)
		return rc;
//[  604.223844] r820t 2-003a: r820t_write: i2c wr reg=0a len=1: db
rc = r82xx_write_reg(priv, 0x0a, 0xdb);
	if (rc < 0)
		return rc;
//[  604.233904] r820t 2-003a: r820t_write: i2c wr reg=0b len=1: 0b
rc = r82xx_write_reg(priv, 0x0b, 0x0b);
	if (rc < 0)
		return rc;
//[  604.247198] r820t 2-003a: r820t_write: i2c wr reg=07 len=1: 75
rc = r82xx_write_reg(priv, 0x07, 0x75);
	if (rc < 0)
		return rc;
//[  604.258941] r820t 2-003a: r820t_write: i2c wr reg=06 len=1: 12
rc = r82xx_write_reg(priv, 0x06, 0x12);
	if (rc < 0)
		return rc;
//[  604.270061] r820t 2-003a: r820t_write: i2c wr reg=1e len=1: 6a
rc = r82xx_write_reg(priv, 0x1e, 0x6a);
	if (rc < 0)
		return rc;
//[  604.280811] r820t 2-003a: r820t_write: i2c wr reg=05 len=1: 03
rc = r82xx_write_reg(priv, 0x05, 0x03);
	if (rc < 0)
		return rc;
//[  604.303440] r820t 2-003a: r820t_write: i2c wr reg=1f len=1: 40
rc = r82xx_write_reg(priv, 0x1f, 0x40);
	if (rc < 0)
		return rc;
//[  604.314872] r820t 2-003a: r820t_write: i2c wr reg=0f len=1: 68
rc = r82xx_write_reg(priv, 0x0f, 0x68);
	if (rc < 0)
		return rc;
//[  604.325838] r820t 2-003a: r820t_write: i2c wr reg=19 len=1: ec
rc = r82xx_write_reg(priv, 0x19, 0xec);
	if (rc < 0)
		return rc;
//[  604.327920] r820t 2-003a: set r820t range#18 for frequency 526 MHz

//[  604.347724] r820t 2-003a: r820t_write: i2c wr reg=17 len=1: 30
rc = r82xx_write_reg(priv, 0x17, 0x30);
	if (rc < 0)
		return rc;
//[  604.371841] r820t 2-003a: r820t_write: i2c wr reg=1a len=1: 61
rc = r82xx_write_reg(priv, 0x1a, 0x61);
	if (rc < 0)
		return rc;
//[  604.400672] r820t 2-003a: r820t_write: i2c wr reg=1b len=1: 00
rc = r82xx_write_reg(priv, 0x1b, 0x00);
	if (rc < 0)
		return rc;
//[  604.422108] r820t 2-003a: r820t_write: i2c wr reg=10 len=1: 6c
rc = r82xx_write_reg(priv, 0x10, 0x6c);
	if (rc < 0)
		return rc;
//[  604.443922] r820t 2-003a: r820t_write: i2c wr reg=08 len=1: e0
rc = r82xx_write_reg(priv, 0x08, 0xe0);
	if (rc < 0)
		return rc;
//[  604.453743] r820t 2-003a: r820t_write: i2c wr reg=09 len=1: 61
rc = r82xx_write_reg(priv, 0x09, 0x61);
	if (rc < 0)
		return rc;
//[  604.464895] r820t 2-003a: r820t_write: i2c wr reg=10 len=1: 6c
rc = r82xx_write_reg(priv, 0x10, 0x6c);
	if (rc < 0)
		return rc;
//[  604.484568] r820t 2-003a: r820t_write: i2c wr reg=1a len=1: 61
rc = r82xx_write_reg(priv, 0x1a, 0x61);
	if (rc < 0)
		return rc;
//[  604.504408] r820t 2-003a: r820t_write: i2c wr reg=12 len=1: 80
rc = r82xx_write_reg(priv, 0x12, 0x80);
	if (rc < 0)
		return rc;
//[  604.513345] r820t 2-003a: r820t_read: i2c rd reg=00 len=5: 96 ad d2 ff db
char data[10];
	rc = r82xx_read(priv, 0x00, data, 5);
	if (rc < 0)
		return rc;
	fprintf(stderr, "%x %x = 96 ad\n", data[0], data[1]);
//[  604.516363] r820t 2-003a: mix_div=4 div_num=1 vco_fine_tune=1

//[  604.533831] r820t 2-003a: r820t_write: i2c wr reg=10 len=1: 2c
rc = r82xx_write_reg(priv, 0x10, 0x2c);
	if (rc < 0)
		return rc;
//[  604.544627] r820t 2-003a: r820t_write: i2c wr reg=14 len=1: 0d
rc = r82xx_write_reg(priv, 0x14, 0x0d);
	if (rc < 0)
		return rc;
//[  604.554061] r820t 2-003a: r820t_write: i2c wr reg=12 len=1: 80
rc = r82xx_write_reg(priv, 0x12, 0x80);
	if (rc < 0)
		return rc;
//[  604.556519] r820t 2-003a: freq 526599 kHz, pll ref 16000, sdm=0xd32e

//[  604.566360] r820t 2-003a: r820t_write: i2c wr reg=16 len=1: d3
rc = r82xx_write_reg(priv, 0x16, 0xd3);
	if (rc < 0)
		return rc;
//[  604.579458] r820t 2-003a: r820t_write: i2c wr reg=15 len=1: 2e
rc = r82xx_write_reg(priv, 0x15, 0x2e);
	if (rc < 0)
		return rc;
//[  604.603149] r820t 2-003a: r820t_read: i2c rd reg=00 len=3: 96 ad d2
	rc = r82xx_read(priv, 0x00, data, 3);
	if (rc < 0)
		return rc;
	printf("%d %d %d = 96 ad d2\n", data[0], data[1], data[2]);

//[  604.605669] r820t 2-003a: tuner has lock at frequency 526599 kHz

//[  604.614949] r820t 2-003a: r820t_write: i2c wr reg=1a len=1: 69
rc = r82xx_write_reg(priv, 0x1a, 0x69);
	if (rc < 0)
		return rc;
//[  604.617380] r820t 2-003a: adjusting tuner parameters for the standard

//[  604.636997] r820t 2-003a: r820t_write: i2c wr reg=1d len=1: c5
rc = r82xx_write_reg(priv, 0x1d, 0xc5);
	if (rc < 0)
		return rc;
//[  604.657310] r820t 2-003a: r820t_write: i2c wr reg=1c len=1: 24
rc = r82xx_write_reg(priv, 0x1c, 0x24);
	if (rc < 0)
		return rc;
//[  604.677423] r820t 2-003a: r820t_write: i2c wr reg=0d len=1: 53
rc = r82xx_write_reg(priv, 0x0d, 0x53);
	if (rc < 0)
		return rc;
//[  604.696458] r820t 2-003a: r820t_write: i2c wr reg=0e len=1: 75
rc = r82xx_write_reg(priv, 0x0e, 0x75);
	if (rc < 0)
		return rc;
//[  604.707773] r820t 2-003a: r820t_write: i2c wr reg=05 len=1: 03
rc = r82xx_write_reg(priv, 0x05, 0x03);
	if (rc < 0)
		return rc;
//[  604.718261] r820t 2-003a: r820t_write: i2c wr reg=06 len=1: 12
rc = r82xx_write_reg(priv, 0x06, 0x12);
	if (rc < 0)
		return rc;
//[  604.728468] r820t 2-003a: r820t_write: i2c wr reg=11 len=1: bb
rc = r82xx_write_reg(priv, 0x11, 0xbb);
	if (rc < 0)
		return rc;
//[  604.738865] r820t 2-003a: r820t_write: i2c wr reg=17 len=1: 30
rc = r82xx_write_reg(priv, 0x17, 0x30);
	if (rc < 0)
		return rc;
//[  604.751311] r820t 2-003a: r820t_write: i2c wr reg=0a len=1: db
rc = r82xx_write_reg(priv, 0x0a, 0xdb);
	if (rc < 0)
		return rc;
//[  604.754511] r820t 2-003a: adjusting LNA parameters

//[  604.772351] r820t 2-003a: r820t_write: i2c wr reg=1d len=1: c5
rc = r82xx_write_reg(priv, 0x1d, 0xc5);
	if (rc < 0)
		return rc;
//[  604.782645] r820t 2-003a: r820t_write: i2c wr reg=1c len=1: 20
rc = r82xx_write_reg(priv, 0x1c, 0x20);
	if (rc < 0)
		return rc;
//[  604.792789] r820t 2-003a: r820t_write: i2c wr reg=06 len=1: 12
rc = r82xx_write_reg(priv, 0x06, 0x12);
	if (rc < 0)
		return rc;
//[  604.812482] r820t 2-003a: r820t_write: i2c wr reg=1a len=1: 79
rc = r82xx_write_reg(priv, 0x1a, 0x79);
	if (rc < 0)
		return rc;
//[  605.097355] r820t 2-003a: r820t_write: i2c wr reg=1d len=1: dd
rc = r82xx_write_reg(priv, 0x1d, 0xdd);
	if (rc < 0)
		return rc;
//[  605.111321] r820t 2-003a: r820t_write: i2c wr reg=1c len=1: 24
rc = r82xx_write_reg(priv, 0x1c, 0x24);
	if (rc < 0)
		return rc;
//[  605.134707] r820t 2-003a: r820t_write: i2c wr reg=1e len=1: 6e
rc = r82xx_write_reg(priv, 0x1e, 0x6e);
	if (rc < 0)
		return rc;
//[  605.144965] r820t 2-003a: r820t_write: i2c wr reg=1a len=1: 69
rc = r82xx_write_reg(priv, 0x1a, 0x69);
	if (rc < 0)
		return rc;

//[  605.153290] r820t 2-003a: r820t_read: i2c rd reg=00 len=4: 96 ad d2 ff
	rc = r82xx_read(priv, 0x00, data, 4);
	if (rc < 0)
		return rc;
	printf("%d %d %d %d = 96 ad d2 ff\n", data[0], data[1], data[2], data[3]);

//[  605.157106] r820t 2-003a: generic_set_freq: PLL locked on frequency 522029969 Hz, gain=31
...
static int r82xx_set_pll2(struct r82xx_priv *priv, uint32_t freq)
{
	int rc, i;
...
int r82xx_init(struct r82xx_priv *priv)
{
	...
	rc = r82xx_sysfreq_sel(priv, 0, TUNER_DIGITAL_TV, SYS_DVBT2);
