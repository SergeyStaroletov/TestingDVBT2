...
void QGCMapWidget::guidedActionTriggered()
{
    ...
    bool aslAglChanged = false; //defaultGuidedFrame != currWPManager->getFrameRecommendation();

    if ( aslAglChanged || !defaultGuidedAltFirstTimeSet)
    {
        defaultGuidedAltFirstTimeSet = true; // so we don't prompt again.
        QString altFrame;
        defaultGuidedFrame = 0;// currWPManager->getFrameRecommendation();
...
void QGCMapWidget::guidedAltActionTriggered()
{
    ...
    Waypoint wp;
    double tmpAlt;
    if(false//  defaultGuidedFrame != currWPManager->getFrameRecommendation()
            ){
...
void QGCMapWidget::updateHomePosition(double latitude, double longitude, double altitude)
{
    ...
    SetShowHome(false);                      // display the HOME position on the map
