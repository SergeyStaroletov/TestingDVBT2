...
extern bool stopped_analize;
bool should_watch_data;

class DvbPatEntry {
public:
  DvbPatEntry(int programNumber_, int pid_)
      : programNumber(programNumber_), pid(pid_) {}
  ~DvbPatEntry() {}
...
class DvbSdtEntry {
public:
  DvbSdtEntry(int serviceId_, int networkId_, bool scrambled_)
      : serviceId(serviceId_), networkId(networkId_), scrambled(scrambled_) {}
  ~DvbSdtEntry() {}
...
class PidScanFilter : public DvbSectionFilter, QObject {
public:
  PidScanFilter(PidScan *scan_, bool useOtherNit_)
      : scan(scan_), pid(-1), useOtherNit(useOtherNit_) {}

  ~PidScanFilter() { stopFilter(); }

  bool isActive() const { return (pid != -1); }
  DvbScan::FilterType mytype() { return type; }

  bool startFilter(int pid_, DvbScan::FilterType type_, int timeout);
  void stopFilter();
...
bool PidScanFilter::startFilter(int pid_, DvbScan::FilterType type_,
                                int timeout) {
  Q_ASSERT(pid == -1);
  ...
  // FIXME check timings
  if (timeout != -1 && type != DvbScan::NitFilter) {
    // timerId = startTimer(timeout);
  } else {
    // timerId = startTimer(4 * timeout);
  }

  return true;
}

void PidScanFilter::stopFilter() {
  if (pid != -1) {
    // killTimer(timerId);
...
bool PidScanFilter::checkMultipleSection(const DvbStandardSection &section) {
...
bool PidScanFilter::isFinished() {
  /*for (int i = 0; i < multipleSections.size(); i++) {
    if (multipleSections[i].check.count(false) != 0)
      return false;
  }*/
  return stopped_analize;
}

void PidScanFilter::processSection(const char *data, int size) {

  qDebug() << "section pid= " << pid << " size= " << size;
  ...
  case DvbScan::NitFilter: {
    ...
    if (!((nitSection.tableId() == 0x40) ||
          (useOtherNit && (nitSection.tableId() == 0x41))))
      return;

    qCDebug(logDvb, "Handling NIT table ID 0x%02x, extension 0x%04x",
            nitSection.tableId(), nitSection.tableIdExtension());
...
void PidScanFilter::timerEvent(QTimerEvent *) {
  qCWarning(logDvb, "Timeout while reading section; type = %d, PID = %d", type,
            pid);
  // scan->filterFinished(this);
}

PidScan::PidScan(DvbDevice *device_, const QString &source_,
                 const DvbTransponder &transponder_, bool useOtherNit_)
    : device(device_), source(source_), transponder(transponder_), isLive(true),
      isAuto(false), useOtherNit(useOtherNit_), transponderIndex(-1),
      state(ScanPat), patIndex(0), activeFilters(0) {
  qCDebug(logDvb, "Use other NIT is %s", useOtherNit ? "enabled" : "disabled");
}

PidScan::PidScan(DvbDevice *device_, const QString &source_,
                 const QList<DvbTransponder> &transponders_, bool useOtherNit_)
    : device(device_), source(source_), isLive(false), isAuto(false),
      useOtherNit(useOtherNit_), transponders(transponders_),
      transponderIndex(0), state(ScanTune), patIndex(0), activeFilters(0) {
  qCDebug(logDvb, "Use other NIT is %s", useOtherNit ? "enabled" : "disabled");
}

PidScan::PidScan(DvbDevice *device_, const QString &source_,
                 const QString &autoScanSource, bool useOtherNit_)
    : device(device_), source(source_), isLive(false), isAuto(true),
      useOtherNit(useOtherNit_), transponderIndex(0), state(ScanTune),
      patIndex(0), activeFilters(0) {
  qCDebug(logDvb, "Use other NIT is %s", useOtherNit ? "enabled" : "disabled");
}

PidScan::~PidScan() { qDeleteAll(filters); }
...
void PidScan::deviceStateChanged() {
...
bool PidScan::startFilter(int pid, DvbScan::FilterType type) {
  qDebug() << "[!!] Starting filter on pid = " << pid;
  if (activeFilters != filters.size()) {
    foreach (PidScanFilter *filter, filters) {
      if (!filter->isActive()) {
        int timeout = -1;
        if (filter->mytype() == DvbScan::FilterType::OtherFilter)
          timeout = -1;
        else
          timeout = 5000;
        if (!filter->startFilter(pid, type, timeout)) {
          return false;
        }
...
void PidScan::updateState() {
  ...
  while (true) {
    ...
    case ScanPat: {
      ...
      if (!startFilter(0x0, DvbScan::PatFilter)) {
        return;
      }

      /*
      startFilter(2054, OtherFilter);
      startFilter(2055, OtherFilter);
      startFilter(2056, OtherFilter);
      startFilter(2057, OtherFilter);
      startFilter(2058, OtherFilter);
    */
    ...
    case ScanNit: {
      ...
      if (!isLive && !isAuto &&
        ...
        if (!startFilter(0x10, DvbScan::NitFilter)) {
          return;
        }
      }
    ...
    case ScanSdt: {
      ...
      if (transponder.getTransmissionType() != DvbTransponderBase::Atsc) {
        if (!startFilter(0x11, DvbScan::SdtFilter)) {
          return;
        }
      } else {
        if (!startFilter(0x1ffb, DvbScan::VctFilter)) {
          return;
        }
      }
    ...
    case ScanPmt: {
      ...
      while (patIndex < patEntries.size()) {
        if (!startFilter(patEntries.at(patIndex).pid, DvbScan::PmtFilter)) {
          // return;
        }
      ...
      if (isLive) {
        qCInfo(logDvb,
               "Scanning while live stream. Can't change the transponder");
        // emit scanFinished();
        // return;
      }
      /*
    ...
    */
    ...
    case ScanTune: {
      ...
      qCDebug(logDvb, "Transponder %d/%d", transponderIndex,
              transponders.size());
...
void PidScan::processPat(const DvbPatSection &section) {
  transportStreamId = section.transportStreamId();

  for (DvbPatSectionEntry entry = section.entries(); entry.isValid();
       entry.advance()) {
    if (entry.programNumber() != 0x0) {
      ...
      qDebug() << "pat entry num=" << entry.programNumber()
               << " pid=" << entry.pid();

      qCDebug(logDvb, "New PAT entry: pid %d, program %d", entry.pid(),
              entry.programNumber());
    }
  }
}

void PidScan::processPmt(const DvbPmtSection &section, int pid) {
  DvbPreviewChannel channel;

  for (auto entry = section.entries(); entry.isValid(); entry.advance()) {
    qDebug() << "PROCESS PMT PID" << entry.pid();
    //
    startFilter(entry.pid(), DvbScan::FilterType::OtherFilter);
  };
  ...
  if (channel.hasVideo || (channel.audioPid >= 0)) {
    ...
    qCDebug(logDvb, "New channel: PID %d, service ID %d", pid,
            section.programNumber());
  }
}

void PidScan::processSdt(const DvbSdtSection &section) {
  for (DvbSdtSectionEntry entry = section.entries(); entry.isValid();
       entry.advance()) {
    ...
    for (DvbDescriptor descriptor = entry.descriptors(); descriptor.isValid();
      ...
      qCDebug(logDvb,
              "New SDT entry: service ID 0x%04x, name '%s', provider '%s'",
              entry.serviceId(), qPrintable(sdtEntry.name),
              qPrintable(sdtEntry.provider));
      sdtEntries.append(sdtEntry);
    }
  }
}

void PidScan::processVct(const AtscVctSection &section) {
  ...
  for (int i = 0; i < entryCount && (entry.isValid()); i++) {
    QString majorminor = QString(QLatin1String("%1-%2 "))
                             .arg(entry.majorNumber(), 3, 10, QLatin1Char('0'))
                             .arg(entry.minorNumber());

    DvbSdtEntry sdtEntry(entry.programNumber(), entry.sourceId(),
                         entry.isScrambled());
    ...
    if (sdtEntry.name.isEmpty()) {
      ...
      // to the short name
      QChar shortName[] = {entry.shortName1(), entry.shortName2(),
                           entry.shortName3(), entry.shortName4(),
                           entry.shortName5(), entry.shortName6(),
                           entry.shortName7(), 0};
      int nameLength = 0;
...
void PidScan::processNit(const DvbNitSection &section) {
  for (DvbNitSectionEntry entry = section.entries(); entry.isValid();
       entry.advance()) {
    for (DvbDescriptor descriptor = entry.descriptors(); descriptor.isValid();
         descriptor.advance()) {
...
static DvbCTransponder::Modulation
extractDvbCModulation(const DvbCableDescriptor &descriptor) {
...
static DvbCTransponder::FecRate
extractDvbCFecRate(const DvbCableDescriptor &descriptor) {
...
static DvbSTransponder::Polarization
extractDvbSPolarization(const DvbSatelliteDescriptor &descriptor) {
...
static DvbSTransponder::FecRate
extractDvbSFecRate(const DvbSatelliteDescriptor &descriptor) {
  switch (descriptor.fecRate()) {
  case 1:
    return DvbTransponderBase::Fec1_2;
  case 2:
...
static DvbS2Transponder::Modulation
extractDvbS2Modulation(const DvbSatelliteDescriptor &descriptor) {
  switch (descriptor.modulation()) {
  case 1:
...
static DvbS2Transponder::RollOff
extractDvbS2RollOff(const DvbSatelliteDescriptor &descriptor) {
...
static DvbTTransponder::Bandwidth
extractDvbTBandwidth(const DvbTerrestrialDescriptor &descriptor) {
...
static DvbTTransponder::Modulation
extractDvbTModulation(const DvbTerrestrialDescriptor &descriptor) {
...
static DvbTTransponder::FecRate
extractDvbTFecRateHigh(const DvbTerrestrialDescriptor &descriptor) {
...
static DvbTTransponder::FecRate
extractDvbTFecRateLow(const DvbTerrestrialDescriptor &descriptor) {
...
static DvbTTransponder::TransmissionMode
extractDvbTTransmissionMode(const DvbTerrestrialDescriptor &descriptor) {
...
static DvbTTransponder::GuardInterval
extractDvbTGuardInterval(const DvbTerrestrialDescriptor &descriptor) {
...
static DvbTTransponder::Hierarchy
extractDvbTHierarchy(const DvbTerrestrialDescriptor &descriptor) {
...
void PidScan::processNitDescriptor(const DvbDescriptor &descriptor) {
  ...
  case DvbTransponderBase::DvbC: {
    ...
    qCDebug(logDvb, "Added transponder: %.2f MHz",
            dvbCTransponder->frequency / 1000000.);
    break;
  }
  ...
  case DvbTransponderBase::DvbS2: {
    ...
    if (!satelliteDescriptor.isDvbS2()) {
      ...
      DvbS2Transponder *dvbS2Transponder =
          newTransponder.as<DvbS2Transponder>();
      dvbS2Transponder->modulation =
          extractDvbS2Modulation(satelliteDescriptor);
    ...
    dvbSTransponder->polarization =
        extractDvbSPolarization(satelliteDescriptor);
    ...
    qCDebug(logDvb, "Added transponder: %.2f MHz",
            dvbSTransponder->frequency / 1000000.);
    break;
  }
  ...
  case DvbTransponderBase::DvbT: {
    ...
    dvbTTransponder->fecRateHigh =
        extractDvbTFecRateHigh(terrestrialDescriptor);
    ...
    dvbTTransponder->guardInterval =
        extractDvbTGuardInterval(terrestrialDescriptor);
    ...
    qCDebug(logDvb, "Added transponder: %.2f MHz",
            dvbTTransponder->frequency / 1000000.);
    break;
  }
  ...
  case DvbTransponderBase::IsdbT:
    ...
    for (int i = 0; i < IsdbTDescriptor.frequencyLength(); i++) {
      ...
      IsdbTTransponder *isdbTTransponder =
          newTransponder.as<IsdbTTransponder>();

      isdbTTransponder->frequency = (uint32_t)(
          (((uint64_t)IsdbTDescriptor.frequency(i)) * 1000000ul) / 7);
      isdbTTransponder->bandwidth = IsdbTTransponder::Bandwidth6MHz;
      isdbTTransponder->transmissionMode =
          IsdbTTransponder::TransmissionModeAuto;
      ...
      qCDebug(logDvb, "Added transponder: %.2f MHz",
              isdbTTransponder->frequency / 1000000.);
    }
    return;
  }
...
void PidScan::filterFinished(PidScanFilter *filter) {
  filter->stopFilter();
  --activeFilters;
  updateState();
}
