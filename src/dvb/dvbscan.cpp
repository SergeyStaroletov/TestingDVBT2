...
public:
  DvbScanFilter(DvbScan *scan_, bool useOtherNit_)
      : scan(scan_), pid(-1), useOtherNit(useOtherNit_) {}

  ~DvbScanFilter() { stopFilter(); }
...
DvbScan::DvbScan(DvbDevice *device_, const QString &source_,
  ...
  if ((autoScanSource == QLatin1String("AUTO-T-Normal")) ||
    ...
    for (int frequency = 177500000; frequency <= 226500000;
         frequency += 7000000) {
    ...
    for (int frequency = 474000000; frequency <= 858000000;
         frequency += 8000000) {
      for (int i = 0; i < 3; ++i) {
        if ((i != 0) && (!offsets)) {
          break;
    ...
    for (int frequency = 529500000; frequency <= 816500000;
         frequency += 7000000) {
      for (int i = 0; i < 2; ++i) {
        int offset = 0;
    ...
    for (unsigned i = 0; i < (sizeof(italyVhf) / sizeof(italyVhf[0])); ++i) {
      for (int j = 0; j < 2; ++j) {
        ...
        dvbTTransponder->bandwidth =
            ((j == 0) ? DvbTTransponder::Bandwidth7MHz
    ...
    for (int frequency = 527000000; frequency <= 599000000;
         frequency += 6000000) {
      DvbTransponder currentTransponder(DvbTransponderBase::DvbT);
      DvbTTransponder *dvbTTransponder =
          currentTransponder.as<DvbTTransponder>();
  ...
  if ((autoScanSource == QLatin1String("AUTO-T2-Normal")) ||
    ...
    for (unsigned i = 0; i < (sizeof(italyVhf) / sizeof(italyVhf[0])); ++i) {
      for (int j = 0; j < 2; ++j) {
        ...
        dvbT2Transponder->bandwidth =
            ((j == 0) ? DvbT2Transponder::Bandwidth7MHz
  ...
  if (autoScanSource == QLatin1String("AUTO-UHF-6MHz")) {
    for (int frequency = 473142857; frequency <= 803142857;
         frequency += 6000000) {
...
DvbScan::~DvbScan() { qDeleteAll(filters); }
...
void DvbScan::processPat(const DvbPatSection &section) {
  ...
  for (DvbPatSectionEntry entry = section.entries(); entry.isValid();
    ...
    if (entry.programNumber() != 0x0) {
      ...
      // qDebug() << "pnum=" << entry.programNumber() << " pid=" << entry.pid();
