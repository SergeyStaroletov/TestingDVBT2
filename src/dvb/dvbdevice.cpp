...
public:
  DvbSectionFilterInternal()
      : activeSectionFilters(0), continuityCounter(0), wrongCrcIndex(0),
        bufferValid(false) {
...
void DvbSectionFilterInternal::processData(const char data[188]) {
  ...
  if (bufferValid) {
    if (continuity == continuityCounter) {
      qCDebug(logDvb, "Section duplication: received %d, expecting %d",
              continuity, continuityCounter + 1);
      return;
    }

    if (continuity != ((continuityCounter + 1) & 0x0f)) {
      qCDebug(logDvb, "Section discontinuity: received %d, expecting %d",
              continuity, continuityCounter + 1);
      bufferValid = false;
    }
  }
...
void DvbSectionFilterInternal::processSections(bool force) {
  ...
  while (it != end) {
    ...
    const char *sectionEnd =
        (it +
         (((static_cast<unsigned char>(it[1]) & 0x0f) << 8) |
...
DvbDataDumper::DvbDataDumper() {
  ...
  setFileName(QDir::homePath() + QLatin1String("/KaffeineDvbDump-") +
              QString::number(rand, 16) + QLatin1String(".bin"));
...
void DvbDataDumper::processData(const char data[188]) { write(data, 188); }

DvbDevice::DvbDevice(DvbBackendDevice *backend_, QObject *parent)
    : QObject(parent), backend(backend_), deviceState(DeviceReleased),
      dataDumper(NULL), cleanUpFilters(false), isAuto(false),
      unusedBuffersHead(NULL), usedBuffersHead(NULL), usedBuffersTail(NULL) {
...
QString DvbDevice::getDeviceId() const { return backend->getDeviceId(); }
...
void DvbDevice::tune(const DvbTransponder &transponder) {
  DvbTransponderBase::TransmissionType transmissionType =
  ...
  case DvbConfigBase::UsalsRotor: {
    ...
    if (!ok)
      qCWarning(logDev, "Can't extract orbital position from %s",
                qPrintable(config->scanSource));
    ...
    double angle =
        (temp2 + atan(sin(temp2) / ((semiMajorAxis / temp) - cos(temp2))));
    int value = 0;
...
void DvbDevice::startDescrambling(const QByteArray &pmtSectionData,
                                  QObject *user) {
...
void DvbDevice::stopDescrambling(const QByteArray &pmtSectionData,
                                 QObject *user) {
  DvbPmtSection pmtSection(pmtSectionData);
...
bool DvbDevice::isTuned() const { return backend->isTuned(); }
...
void DvbDevice::frontendEvent() {
  DvbTransponderBase::TransmissionType transmissionType =
      autoTransponder.getTransmissionType();
...
void DvbDevice::stop() {
  ...
  for (QMap<int, DvbSectionFilterInternal>::ConstIterator it =
           sectionFilters.constBegin();
...
void DvbDevice::writeBuffer(const DvbDataBuffer &dataBuffer) {
  DvbDeviceDataBuffer *buffer =
      reinterpret_cast<DvbDeviceDataBuffer *>(dataBuffer.data);
