...
public:
  explicit DvbPreviewChannelTableModel(QObject *parent)
      : QAbstractTableModel(parent) {}
  ~DvbPreviewChannelTableModel() {}

  enum ItemDataRole { DvbPreviewChannelRole = Qt::UserRole };
  ...
  QList<DvbPreviewChannel> getChannels() const { return channels; }
...
QAbstractItemModel *
DvbPreviewChannelTableModel::createProxyModel(QObject *parent) {
...
QVariant DvbPreviewChannelTableModel::data(const QModelIndex &index,
                                           int role) const {
...
QVariant DvbPreviewChannelTableModel::headerData(int section,
...
DvbScanDialog::DvbScanDialog(DvbManager *manager_, QWidget *parent)
    : QDialog(parent), manager(manager_), internal(NULL) {
  ...
  channelView->sortByColumn(header->sortIndicatorSection(),
                            header->sortIndicatorOrder());
  channelView->setSortingEnabled(true);
  channelTableModel->setChannelModel(channelModel);
  connect(channelTableModel, &DvbChannelTableModel::checkChannelDragAndDrop,
          channelView, &DvbChannelView::checkChannelDragAndDrop);

  QAction *action = channelView->addEditAction();
  QPushButton *pushButton =
      new QPushButton(action->icon(), action->text(), groupBox);
  connect(pushButton, &QPushButton::clicked, channelView,
          &DvbChannelView::editChannel);
  boxLayout->addWidget(pushButton);

  action = channelView->addRemoveAction();
  pushButton = new QPushButton(action->icon(), action->text(), groupBox);
  connect(pushButton, &QPushButton::clicked, channelView,
          &DvbChannelView::removeChannel);
  boxLayout->addWidget(pushButton);

  pushButton =
      new QPushButton(QIcon::fromTheme(QLatin1String("edit-clear-list"),
                                       QIcon(":edit-clear-list")),
                      i18nc("remove all items from a list", "Clear"), groupBox);
  connect(pushButton, &QPushButton::clicked, channelView,
          &DvbChannelView::removeAllChannels);
  boxLayout->addWidget(pushButton);
  ...
  otherNitCheckBox =
      new QCheckBox(i18n("Search transponders for other Networks"), groupBox);
  otherNitCheckBox->setWhatsThis(
      i18n("On certain networks, it is possible that some transponders are "
           "encoded on separate Network Information Tables (other NITs). This "
           "is more common on DVB-C systems. Clicking on this icon will change "
           "the scan algorithm to take those other NIT data into account. "
           "Please notice that the scan will be a lot more slow if enabled."));
  groupLayout->addWidget(otherNitCheckBox);

  scanButton = new QPushButton(
      QIcon::fromTheme(QLatin1String("edit-find"), QIcon(":edit-find")),
      i18n("Start Scan"), groupBox);
  scanButton->setCheckable(true);
  connect(scanButton, &QPushButton::clicked, this,
          &DvbScanDialog::scanButtonClicked);
...
void DvbScanDialog::scanButtonClicked(bool checked) {
  ...
  connect(internal, &DvbScan::foundChannels, this,
          &DvbScanDialog::foundChannels);
  connect(internal, &DvbScan::scanProgress, progressBar,
          &QProgressBar::setValue);
  // calling scanFinished() will delete internal, so we have to queue the
  // signal!
  connect(internal, &DvbScan::scanFinished, this, &DvbScanDialog::scanFinished,
          Qt::QueuedConnection);
...
void DvbScanDialog::foundChannels(const QList<DvbPreviewChannel> &channels) {
  ...
  foreach (const DvbPreviewChannel &channel, channels) {
    ...
    QStringList::const_iterator it =
        std::lower_bound(providers.constBegin(), providers.constEnd(),
                         channel.provider, localeAwareLessThan2);
